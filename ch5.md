...
找到这样的并行模型的一个方法是构造一个更加的关系E，这个关系以子集的形式包含顺序关系$<_S$。即$(<_S) \subseteq E$
...
为了利用这样的局部并行性，局部并行性可以构造这样的一个局部布局，使得其等价关系满足（5.20），当限制在尚未被节点的祖先分开的语句实例对时。