...
多空间表达式将具有不同域和/或范围空间的分段表达式组合成一个表达式，但是domain不相交的组成成单个表达式。多空间表达式没有特定的域或范围空间，即使所有组成分段表达式碰巧具有相同的域或范围空间。多空间表达式的域是组合分段表达式的域的并集。整数元组 x 处的多空间表达式的值是 x 处的分段表达式的值，该表达式在其域中包含 x，如果有的话。
...
并且说明了在`isl_multi_aff`API中，元组的空间就是range space。
...
在符号表示上，拟仿射表达式分段元组中包含一堆带有条件的拟仿射表达式元组，用分号分隔，每个带条件的拟仿射表达式元组的形式都和Notation 4.6一样。API使用`isl_pw_multi_aff`

是不是可以理解成，同一个元组内的表达式要求至少domain space都相同?

定义4.17介绍了一种表达式，多空间表达式，该表达式结合了带有不同domain space和range space的分段表达式（中间有一句解释没太明白but with pair-wise disjoint domains into a single expression.) 多空间表达式没有特定的domain space以及range space。
该表达式的domain就是所有其中分段表达式的并集。
后面有一句关于多空间表达式中元组x的值的描述没太理解.

`isl_union_pw_aff` API描述为将多空间表达式应用到分段拟仿射表达式上。在符号上写法和分段拟仿射表达式相同，但是domain space可以不相同。
